...
import {
  ...
  StudioMemberInputDto,
  StudioRole,
  StudioMemberDto,
  UserInputDto
} from 'src/app/service/studiomembers.service';
...
export class UsersComponent implements OnInit {
  ...
  // Modifica
  editingId: string | null = null;   // studioMemberId in modifica
  ...
  constructor(private membersSvc: StudioMembersService) { }
  ...
  load(): void {
    ...
    this.membersSvc.listMembers().subscribe({
      ...
      error: err => { console.error('Errore nel caricamento dei membri', err); this.users = []; this.filtered = []; this.loading = false; }
    });
  }

  trackById(_: number, u: any): string | number { return u?.id ?? _; }
  ...
  save(): void {
    ...
    this.membersSvc.createMember(input).subscribe({
      next: _ => { this.saving = false; this.modalVisible = false; this.reload(); },
      error: e => { console.error('Errore durante la creazione del membro', e); this.saving = false; }
    });
  }
  ...
  view(u: { id?: string }): void {
    if (!u?.id) { console.warn('[Users] view(): manca studioMemberId nella riga', u); return; }
    this.loading = true;
    this.membersSvc.getMember(u.id).subscribe({
      next: (dto) => { this.viewing = dto; this.detailsVisible = true; this.loading = false; },
      error: (err) => { console.error('[Users] getMember errore', err); this.loading = false; }
    });
  }

  closeDetails(): void { this.detailsVisible = false; this.viewing = null; }

  /* ---------- Modifica ---------- */

  edit(u: { id?: string }): void {
    if (!u?.id) { console.warn('[Users] edit(): manca studioMemberId nella riga', u); return; }
    this.loading = true;
    this.membersSvc.getMember(u.id).subscribe({
      ...
      error: err => { console.error('[Users] getMember (per edit) errore', err); this.loading = false; }
    });
  }
  ...
  chooseEdit(mode: EditMode) { this.editMode = mode; }
  ...
  confirmEdit(): void {
    ...
    if (this.editMode === 'personal') {
      // 🔹 aggiorna SOLO dati personali
      const userPayload: UserInputDto = {
        firstName: f.firstName,
        lastName: f.lastName,
        email: f.email,
      ...
      this.saving = true;
      this.membersSvc.updatePersonalInformation(String(this.editingId), userPayload).subscribe({
        next: () => { this.afterEditSuccess(); },
        error: err => { console.error('[Users] updatePersonalInformation error', err); this.saving = false; }
      });

    } else if (this.editMode === 'role') {
      // 🔹 aggiorna SOLO ruolo (prendo dal primo membership)
      const role = (f.memberships?.[0]?.role as StudioRole) ?? this.viewing?.role ?? 'BACKOFFICE';
      this.saving = true;
      this.membersSvc.updateRole(String(this.editingId), role).subscribe({
        next: () => { this.afterEditSuccess(); },
        error: err => { console.error('[Users] updateRole error', err); this.saving = false; }
      });
    }
  }

  private afterEditSuccess() {
    this.saving = false;
  ...
  askDeleteFromDetails(v: StudioMemberDto): void {
    if (!v?.id) return;
    this.toDelete = { id: v.id, firstName: v.user?.firstName, lastName: v.user?.lastName } as any;
    this.confirmVisible = true;
  }
  ...
  confirmDelete(): void {
    ...
    this.membersSvc.deleteMember(String(this.toDelete.id)).subscribe({
      ...
      error: (err) => { console.error('Errore durante la cancellazione del membro', err); this.saving = false; }
    });
  }
  ...
  private blankForm() {
    return { firstName: '', lastName: '', email: '', phone: '', memberships: [] as MembershipForm[] };
  }
